...
export const categories = ["All", "Building Blocks", "Action Figures", "Board Games", "Plushies", "Gadgets", "Arts & Crafts"]
...
export function calcTotals(cart, discountRate = 0) {
    ...
    <nav class="navbar">
      <div class="navbar-inner">
        ...
        <div class="nav-search">
          <input id="nav-search-input" type="text" placeholder="Search toys, gifts…" autocomplete="off" />
        ...
        <div class="nav-actions">
          <a href="/pages/wishlist.html" class="icon-btn" title="Wishlist">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/>
            </svg>
            <span class="badge badge-pulse" style="background:#ec4899" id="nav-wishlist-count">${wishlistCount}</span>
          </a>

          <a href="/pages/cart.html" class="icon-btn" title="Cart">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/>
              <path d="M1 1h4l2.68 13.39a2 2 0 001.99 1.61h9.72a2 2 0 001.99-1.61L23 6H6"/>
            </svg>
            <span class="badge badge-pulse" style="background:#a855f7" id="nav-cart-count">${cartCount}</span>
          </a>

          <a href="/pages/login.html" class="btn-primary" style="padding:0.5rem 1.1rem;font-size:0.85rem">Login</a>
        </div>
      </div>
    </nav>
  `

  // Update on cart/wishlist changes
  window.addEventListener('tv:cart-updated', () => {
    const c = getCart().reduce((s, i) => s + i.quantity, 0)
    document.getElementById('nav-cart-count').textContent = c
  })
  window.addEventListener('tv:wishlist-updated', () => {
    const w = getWishlist().length
    document.getElementById('nav-wishlist-count').textContent = w
  })
}
  ...
  return `
    <div class="product-card" data-id="${product.id}">
      <div class="product-card-img">
        ...
        ${pct > 0 ? `<div class="product-badge">-${pct}%</div>` : ''}
      </div>
      <div class="product-card-body">
        ...
        <div class="product-price">
          ...
          ${pct > 0 ? `<span class="price-original">$${product.originalPrice.toFixed(2)}</span>` : ''}
        </div>
        <div class="product-actions">
          <button class="btn-add" data-action="add" data-id="${product.id}">Add to Cart</button>
          <button class="btn-wish ${inWish ? 'active' : ''}" data-action="wish" data-id="${product.id}">${inWish ? '❤️' : '🤍'}</button>
        </div>
      </div>
    </div>
  `
